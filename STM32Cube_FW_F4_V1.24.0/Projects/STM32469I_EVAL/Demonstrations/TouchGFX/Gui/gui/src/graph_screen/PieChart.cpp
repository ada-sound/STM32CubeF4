...
void PieChart::setSelected(int index, bool value)
{
    ...
    if (value == selected[index]) return;
...
{
    animationEndedAction = &callback; animationEndedCallbackActive = true;
}
        ...
        case SELECT_DESELECT:
            {
                ...
                for (int i = 0; i < NUM_ITEMS; i++)
                {
                    switch (expand[i])
                    {
                    case -1: displacement[i] = hideRadius; break;
                    case 1: displacement[i] = showRadius; break;
                    }
                }
                recalculatePieChart();
            }
